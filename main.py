...
df['lost_units']   = df.apply(
    lambda x: max(0, x['units_sold'] - x['stock_available']) if x['understock_flag'] else 0, axis=1)
...
df['excess_units']   = df.apply(
    lambda x: max(0, x['stock_available'] - x['units_sold']) if x['overstock_flag'] else 0, axis=1)
holding_rate = 0.10

df['holding_cost'] = (
    df['excess_units'] * df['unit_price'] * holding_rate * df['restock_lead_days']
)
...
# Menghitung roi_value (%) baris menggunakan rumus sederhana:
...
feature_cols = [
    'stock_available', 'unit_margin',
    'refund_rate', 'restock_lead_days'
]
X = df[feature_cols]
y = df['roi_class']
...
model = RandomForestClassifier(
    ...
    max_depth=None,
    random_state=42,
    class_weight='balanced'   # menangani kelas tak seimbang
)

model.fit(X_train, y_train)

# Impor library joblib
import joblib

# Simpan model terlatih ke file model_prediction.pkl menggunakan joblib.dump()
joblib.dump(model, 'model_prediction.pkl')

#Impor library Flask, request, jsonify dari flask
from flask import Flask, request, jsonify

#Inisialisasi server Flask dengan app = Flask(__name__)
app = Flask(__name__)

#Muat model dari file model_prediction.pkl menggunakan joblib dan simpan dalam nama model_api
model_api = joblib.load('model_prediction.pkl')

#Buat endpoint /predict dengan metode POST
@app.route('/predict', methods=['POST'])
def predict():
    # Ambil data input dari JSON, konversi ke DataFrame
    data = request.get_json()
    df = pd.DataFrame([data])

    # Gunakan model untuk prediksi
    prediction = model_api.predict(df)
